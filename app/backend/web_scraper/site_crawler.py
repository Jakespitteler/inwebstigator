...
async def fetch_internal_links_from_url(
    ...
    Returns:
        CrawlFetchResult: (the url, all of the internal links on that page, the status
    ...
    async with semaphore:
        try:
            ...
            if not is_internal_web_page(base_url, check_url=absolute_url):
                return CrawlFetchResult(url=url, internal_links=[], status_code=None)
            ...
            return CrawlFetchResult(url=absolute_url, internal_links=links, status_code=200)
            ...
            return CrawlFetchResult(url=url, internal_links=[], status_code=status_code)
...
async def crawl_site(
    ...
    while queue and len(visited) < max_pages:
        ...
        tasks: Iterator[Awaitable[CrawlFetchResult]] = (
        ...
        batch_results: list[CrawlFetchResult | BaseException] = await asyncio.gather(*tasks, return_exceptions=True)
        ...
        for result in batch_results:
            ...
            visited.add(normalise_url(result.url))
